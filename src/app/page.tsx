...
export default async function HomePage() {
  ...
  const taskStatusIcon: Record<string, React.ReactNode> = {
    ...
    'testing': <Loader2 className="w-4 h-4 text-amber-400" />,
      ...
      <div className="glass-card p-8">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-blue-200/70">可视化项目管理与任务追踪，让工作更高效</p>
          </div>
          <div className="text-right">
            <div className="text-4xl font-bold text-white/90">{new Date().toLocaleDateString('zh-CN', { month: 'long', day: 'numeric', weekday: 'long' })}</div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <div className="glass-card p-6 card-appear" style={{ animationDelay: '0ms' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-blue-300/50">总项目</span>
          </div>
          <div className="text-4xl font-bold stat-number">{projects?.length || 0}</div>
          <div className="text-sm text-slate-400 mt-1">项目总数</div>
        </div>
        ...
        <div className="glass-card p-6 card-appear" style={{ animationDelay: '100ms' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-green-300/50">进行中</span>
          </div>
          <div className="text-4xl font-bold text-green-400">{inProgressCount}</div>
          <div className="text-sm text-slate-400 mt-1">活跃项目</div>
        </div>
        ...
        <div className="glass-card p-6 card-appear" style={{ animationDelay: '200ms' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-emerald-300/50">已完成</span>
          </div>
        ...
        <div className="glass-card p-6 card-appear" style={{ animationDelay: '300ms' }}>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-purple-300/50">动态</span>
          </div>
          <div className="text-4xl font-bold text-purple-400">{activities?.length || 0}</div>
          <div className="text-sm text-slate-400 mt-1">最近活动</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="glass-card p-6 card-appear" style={{ animationDelay: '500ms' }}>
          ...
          <div className="space-y-4 relative">
            {/* 时间线竖线 */}
            <div className="absolute left-[15px] top-2 bottom-2 w-px bg-gradient-to-b from-purple-500 via-blue-500 to-cyan-500"></div>
            ...
            {activities && activities.length > 0 ? (
              activities.map((activity, index) => (
                ...
                >
                  {/* 时间线圆点 */}
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-purple-500 to-blue-500 flex items-center justify-center flex-shrink-0 z-10">
