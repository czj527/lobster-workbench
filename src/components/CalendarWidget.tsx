    ...
    return (
      <div className="glass-card p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-medium text-theme-primary">今日日程</h3>
        </div>
    ...
    return (
      <div className="glass-card p-4">
        <div className="flex items-center justify-between mb-3">
          ...
          <button onClick={fetchEvents} className="p-1 hover:bg-theme-list-item rounded">
            <RefreshCw className="w-3 h-3 text-theme-muted" />
          </button>
        </div>
        <p className="text-xs text-theme-muted text-center py-4">{error}</p>
      </div>
    );
  }
  ...
  return (
    <div className="glass-card p-4">
      <div className="flex items-center justify-between mb-3">
        ...
        <button onClick={fetchEvents} className="p-1 hover:bg-theme-list-item rounded transition-colors" title="刷新">
          <RefreshCw className="w-3 h-3 text-theme-muted hover:text-theme-accent" />
        </button>
      </div>

      {Object.keys(eventsByDate).length === 0 ? (
        <p className="text-xs text-theme-muted text-center py-4">暂无日程安排</p>
      ) : (
              ...
              <div className="space-y-1.5">
                {dayEvents.map((event) => {
                  ...
                  return (
                    <div
                      key={event.id}
                      className="flex items-start gap-2 p-2 rounded-lg bg-theme-list-item hover:bg-theme-list-item-hover transition-colors"
                    >
                      <span className={`text-xs px-1.5 py-0.5 rounded ${config.color} flex-shrink-0`}>
                        {config.label}
                      </span>
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-theme-muted">
                          ...
                          {event.payload.end_time && ` - ${event.payload.end_time}`}
                        </p>
                      </div>
                    </div>
                  );
