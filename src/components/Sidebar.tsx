...
import { Home, FolderKanban, Sparkles, Link2 } from "lucide-react";
...
export default function Sidebar({ isOpen, isMobile, onClose }: SidebarProps) {
  ...
  return (
    <>
      {isMobile && <Overlay />}
      <aside className={`fixed left-0 top-0 h-full w-64 glass-sidebar z-50 transition-transform duration-300 ease-in-out ${isMobile ? isOpen ? "translate-x-0" : "-translate-x-full" : "translate-x-0"}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 md:p-6 border-b border-[var(--border-light)]">
            <div className="flex justify-center mb-3 md:mb-4">
              <div className="w-14 h-14 md:w-16 md:h-16 rounded-full overflow-hidden border-2 border-sky-300 dark:border-blue-400 avatar-glow">
                <img src="/avatar.webp" alt="蓝的头像" className="w-full h-full object-cover" />
              </div>
            </div>
            <div className="flex items-center justify-center space-x-2 md:space-x-3">
              <div className="w-7 h-7 md:w-8 md:h-8 rounded-lg bg-gradient-to-br from-sky-400 to-cyan-400 dark:from-blue-500 dark:to-cyan-400 flex items-center justify-center shadow-lg shadow-sky-400/30 dark:shadow-blue-500/30">
                <Sparkles className="w-3 h-3 md:w-4 md:h-4 text-[var(--text-primary)]" />
              </div>
              <div>
                <h1 className="text-base md:text-lg font-bold text-theme-primary">蓝的工作台</h1>
          ...
          <nav className="flex-1 p-3 md:p-4">
            <ul className="space-y-1 md:space-y-2">
              {navLinks.map((link) => {
                ...
                return (
                  <li key={link.href}>
                    <Link href={link.href} onClick={onClose} className={`nav-link min-h-[44px] ${isActive(link.href) ? "active" : ""}`}>
          ...
          <div className="p-3 md:p-4 border-t border-[var(--border-light)]">
